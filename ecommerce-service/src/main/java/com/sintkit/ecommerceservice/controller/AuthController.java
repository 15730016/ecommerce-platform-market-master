...
import com.sintkit.ecommerceservice.config.CurrentUser;
import com.sintkit.ecommerceservice.dto.ApiResponse;
import com.sintkit.ecommerceservice.dto.JwtAuthenticationResponse;
import com.sintkit.ecommerceservice.dto.LocalUser;
import com.sintkit.ecommerceservice.dto.LoginRequest;
import com.sintkit.ecommerceservice.dto.SignUpRequest;
import com.sintkit.ecommerceservice.dto.SignUpResponse;
...
import com.sintkit.ecommerceservice.security.jwt.TokenProvider;
import com.sintkit.ecommerceservice.service.mail.MailService;
import com.sintkit.ecommerceservice.service.useraccount.UserService;
import com.sintkit.ecommerceservice.util.GeneralUtils;
	...
	public ResponseEntity<?> registerUser(@Valid @RequestBody SignUpRequest signUpRequest) {
		try {
			...
			if (signUpRequest.isUsing2FA()) {
				QrData data = qrDataFactory.newBuilder().label(user.getEmail()).secret(user.getSecret()).issuer("SINT Kit").build();
				// Generate the QR code image data as a base64 string which can
				// be used in an <img> tag:
				String qrCodeImageDB = getDataUriForImage(qrGenerator.generate(data), qrGenerator.getImageMimeType());
				return ResponseEntity.ok().body(new SignUpResponse(true, qrCodeImageDB));
			}
