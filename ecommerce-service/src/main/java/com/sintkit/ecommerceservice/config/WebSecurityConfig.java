...
import com.sintkit.ecommerceservice.security.jwt.TokenAuthenticationFilter;
import com.sintkit.ecommerceservice.security.oauth2.*;
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.cors().and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().csrf().disable().formLogin().disable().httpBasic().disable()
				...
				.antMatchers("/",
						...
						"/api/management/**",
						"/api/auth/**",
						"/oauth2/**").permitAll().anyRequest().authenticated().and().oauth2Login().authorizationEndpoint()
